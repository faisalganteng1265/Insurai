...
const STATUS_COLOR: Record<Policy['status'], string> = {
  ...
  triggered: 'bg-[#251410] text-[#cfa45b]',
  verified: 'bg-[#fff4d6] text-[#b07800]',
  paid: 'bg-[#eaf7fb] text-[#308ca0]',
  expired: 'bg-[#171b24] text-[#777782]',
  cancelled: 'bg-[#171b24] text-[#777782]',
}

function PolicyCard({ policy }: { policy: Policy }) {
  return (
    <div className="rounded-[24px] bg-[#11141b] p-5 shadow-[0_10px_30px_rgba(31,35,45,0.07)]">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          <p className="font-black text-[#f5efe5]">{policy.strategyName}</p>
      ...
      <div className="flex items-center justify-between text-xs">
        <span className="font-bold text-[#7f8794]">Expires in {policy.expiresIn}</span>
        <span className="font-mono font-bold text-[#7f8794]">{policy.proofHash}</span>
      </div>
    </div>
  )
}
...
export default function PortfolioPage() {
  ...
  return (
    <div>
      <div className="mb-8">
        <p className="text-xs font-black uppercase tracking-widest text-[#cfa45b]">My Portfolio</p>
        <h1 className="mt-2 text-4xl font-black text-[#f5efe5]">Copier dashboard</h1>
      </div>

      {triggeredPolicies.length > 0 && (
        <div className="mb-6 rounded-2xl bg-[#251410] border border-[#5c2a23] p-4 flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-black text-[#cfa45b]">Claim triggered on {triggeredPolicies[0].strategyName}</p>
          ...
          <Link
            href="/claims"
            className="rounded-full bg-[#b83227] px-4 py-2.5 text-xs font-black text-white whitespace-nowrap"
          >
            View Claim →
          </Link>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-4 mb-8">
        {[
          ...
          <div key={label} className="rounded-[24px] bg-[#11141b] p-5 shadow-[0_10px_30px_rgba(31,35,45,0.07)]">
            ...
            <p className={`mt-1 text-2xl font-black ${String(value).startsWith('-') ? 'text-[#cfa45b]' : String(value).startsWith('+') ? 'text-[#11875d]' : 'text-[#f5efe5]'}`}>
              {value}
            </p>
          </div>
        ))}
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        <div>
          <h2 className="text-lg font-black text-[#f5efe5] mb-4">Active Policies</h2>
          <div className="space-y-3">
            {policies.map(p => <PolicyCard key={p.id} policy={p} />)}
            {policies.length === 0 && (
              <div className="rounded-[24px] bg-[#11141b] p-8 text-center shadow-[0_10px_30px_rgba(31,35,45,0.07)]">
                <p className="text-sm font-bold text-[#7f8794]">No policies yet.</p>
          ...
          <div className="rounded-[28px] bg-[#2b2c34] p-5 space-y-3">
            {attestations.map(att => (
              <div key={att.id} className="rounded-2xl bg-[#11141b]/8 p-4">
                ...
                <p className="text-[10px] font-bold text-[#7f8794] mb-2">{att.timestamp}</p>
